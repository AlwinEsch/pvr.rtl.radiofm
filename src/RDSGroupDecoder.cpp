...
cRDSGroupDecoder::cRDSGroupDecoder(cRadioReceiver* proc) : m_RadioProc(proc)
{
}
  ...
  if (!versionCode)
  {
    ...
    if (AFCode[0] >= 224 && AFCode[0] <= 249)
    {
      ...
      if (m_AF_MethodStarted && !m_AF_MethodFinished)
      {
        ...
        {
          fprintf(stderr,
                  "Radio RDS/RaSS Processor - %i alternative frequencies exists (%i entries)\n",
          ...
          if (freq > 0.0f)
          {
            ...
            else
            {
              ...
              if (!present)
              {
                ...
                fprintf(stderr, "Radio RDS/RaSS Processor - Change in alternative frequencies, "
                                "restarted scan\n");
              }
            }
          }
...
void cRDSGroupDecoder::Decode_Type1___ProgItemNum_SlowLabelCodes(const uint16_t* msgElement,
                                                                 bool VersionABCode)
{
...
void cRDSGroupDecoder::Decode_Type4A__Clock(const uint16_t* msgElement)
{
  double modified_julian_date =
      ((msgElement[BLOCK__B] & 0x03) << 15) | ((msgElement[BLOCK__C] >> 1) & 0x7fff);
 ...
 */
void cRDSGroupDecoder::Decode_Type5___TransparentDataChannels(const uint16_t* msgElement,
                                                              bool versionCode)
{
#ifdef IMPROVE_CHECK2
  ...
  printf("Radio RDS/RaSS Processor %s - RDS group type: %u%c - 04%X 04%X 04%X 04%X\n", __FUNCTION__,
         group_type, versionCode ? 'A' : 'B', msgElement[0], msgElement[BLOCK__B],
         msgElement[BLOCK__C], msgElement[BLOCK__D]);
#endif
}
 ...
 */
void cRDSGroupDecoder::Decode_Type6___InHouseApplications(const uint16_t* msgElement,
                                                          bool versionCode)
{
  (void)msgElement;
  (void)versionCode;
...
void cRDSGroupDecoder::Decode_Type7A__RadioPaging(const uint16_t* msgElement)
{
  ...
  printf("Radio RDS/RaSS Processor %s - RDS group type: %uA - 04%X 04%X 04%X 04%X\n", __FUNCTION__,
         group_type, msgElement[0], msgElement[BLOCK__B], msgElement[BLOCK__C],
         msgElement[BLOCK__D]);
#endif
}
 ...
 */
void cRDSGroupDecoder::Decode_Type14__EnhancedOtherNetworksInfo(const uint16_t* msgElement,
                                                                bool versionCode)
{
#ifdef IMPROVE_CHECK2
  unsigned int group_type = (unsigned int)((msgElement[BLOCK__B] >> 12) & 0xf);
...
void cRDSGroupDecoder::AddStuffingValue(uint8_t value)
{
  ...
  {
    kodi::Log(ADDON_LOG_ERROR, "value reach end of allowed UECP frame size");
    return;
  }
